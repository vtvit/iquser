...
async def iq_handler(event):
    ...
    if query.startswith("فەرمانەکان") and event.query.user_id == iqub.uid:
        IQPIC = gvarstatus("CMD_PIC") or "https://telegra.ph/file/235481c524f1f15b7895b.mp4"
...
@iqub.iq_cmd(pattern=f"فەرمانەکانم(?: |$)(.*)") 
async def iq(event):
...
async def repoiqub(event):
    ...
    if event.reply_to_msg_id:
        await event.get_reply_message()
    response = await iqub.inline_query(TG_BOT, "سەرچاوە")
    await response[0].click(event.chat_id)
    await event.delete()
