    ...
    def filter(self, event):
        ...
        if self.require_admin and not isinstance(event._chat_peer, types.PeerUser):
            ...
            if not is_creator and not is_admin:
                text = "`پێویستم بە ڕێپێدان(ڕۆڵ) بەڕێوبەریە بۆ ئەوەی بتوانم ئەم فەرمانە بەکار بهێنم!`"
...
async def send_message(
    ...
    *,
    ...
    if safecheck:
        ...
        msg = f"ببوورە، من ناتوانم ئەم نامەیە بنێرم لە چاتێکی گشتیدا، لەوانەیە هەندێك زانیاری هەستیاری تێدابێت، بۆیە بیپشکنە __[گرووپی تۆمارکردنی بۆت]({msglink})."
        return await client.sendmessage(
            entity=chatid,
            message=msg,
            send_as=send_as,
...
async def send_file(
    ...
    **kwargs,
    ...
    if safecheck or safe_file_check:
        ...
        msglink = await client.get_msg_link(response)
        msg = f"ببوورە، من ناتوانم ئەم نامەیە بنێرم لە چاتێکی گشتیدا، لەوانەیە هەندێك زانیاری هەستیاری تێدابێت، بۆیە بیپشکنە __[گرووپی قوفڵی بۆت]({msglink})."
        return await client.sendmessage(
            entity=chatid,
            message=msg,
            reply_to=reply_to,
...
async def edit_message(
    ...
    *,
    ...
    if safecheck:
        ...
        msglink = await client.get_msg_link(response)
        msg = f"ببوورە، من ناتوانم ئەم نامەیە بنێرم لە چاتێکی گشتیدا، لەوانەیە هەندێك زانیاری هەستیاری تێدابێت، بۆیە بیپشکنە __[لە گرووپی قوفڵی بۆت]({msglink})."
        return await client.editmessage(
            entity=chatid,
            message=message,
            text=msg,
